...
| 010 | [Client-Side Recipe URL Scraping](../adrs/adr-010-client-side-recipe-scraping.md) | Aug 2026 | Paste-a-URL import scrapes JSON-LD/microdata on-device via a new pure-Kotlin `:recipe-scraper` module; no backend endpoint, no per-site scrapers, unauthenticated `@ScraperHttpClient` prevents auth-token leakage to third-party hosts. Decision 5 (Aug 2026) adds a rendered-DOM fallback — an off-screen `WebView`, then a visible one — for the minority of sites that refuse HTTP clients outright. Decision 6 (Aug 2026) caches the imported hero image on-device via the same fetch-then-WebView ladder, since the same CDNs that block the HTML fetch block their own images too; the cached path is device-local only, not synced (see #132). |
