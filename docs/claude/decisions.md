...
| 013 | [Measurement Unit Conversion](../adrs/adr-013-measurement-unit-conversion.md) | Aug 2026 | A profile setting (`As written` / `Metric` / `Imperial`) re-expresses ingredient amounts **at display time only** — `recipe_ingredients` is never written, so "As written" always restores the original exactly. Dimensional conversion (volume→volume, mass→mass) is the floor; a curated ~40-staple density table in `core/domain/units/IngredientDensity` is the only thing that crosses cups↔grams, marked `≈`, and suppressed for `chopped`/`shredded`/`grated`… names. Counting units (`clove`, `can`, `pinch`, the scraper's `"unit"`) never convert — `UnitNormalizer` models the convertible set and returns `null` for everything else. Cup = US customary 236.59 ml, a stated assumption. Preference is device-local DataStore (`chefai_user_prefs`), not synced, defaulting to `AS_WRITTEN` so no existing library changes on update. |
| 014 | [Households](../adrs/adr-014-households.md) | Sep 2026 | Sharing a meal plan/grocery list with a household is a **scope layer**, not an ownership migration — `recipes.creator_id` is untouched, and only a nullable `meal_plans.householdId` plus a new gap clause in the backend's recipe sync widen visibility. Household membership folds into `UserSession.Authenticated` (`household: AuthenticatedHousehold?`) rather than a parallel state; household CRUD (create/invite/accept/leave) is plain REST, not the sync protocol, and its local cache tables are wholesale-replaced on refresh, never pushed. `shopping_list_checks` becomes a `SyncableCrossRef` (composite-keyed, like `BookmarkedRecipeEntity`) with an explicit `checked` column — landed in the Room v8→v9 migration ahead of its consumer, so unticking still deletes the row until the sync wiring lands in a later change. Existing rows backfill `SYNCED`, not `PENDING` — a pre-existing tick is a historical fact, not a new edit to push. |

## RFCs

| # | Title | Status |
|---|-------|--------|
| 001 | [Offline-First Sync Protocol](../rfcs/rfc-001-offline-first-sync.md) | Superseded by ADR-006, ADR-007, ADR-008 — kept for historical context |
