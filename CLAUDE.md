...
| Recipe search | Done (#151). Full-text search over title/tags/labels. Backend: `GET /api/v1/recipes/search`, Postgres `tsvector`/GIN (generated column, no extension), tag/label match via a CTE, rate-limited. Android: M3 `SearchBar` on Home (expands over the SDUI content, not a list item), debounced (300ms, 3-char minimum) through `RecipeSearchViewModel` → `RecipeSearchRepository`, which maps results straight to the domain `RecipePreview` (no Room write) and falls back to an on-device `LIKE` scan for anonymous sessions or on network failure, with one refresh-then-retry on 401. Results reuse `RecipeListCard` (now takes `isInCollection`/`onSaveToCollection`/`modifier`); tapping saves inline or opens the existing detail screen. Filters (vegetarian, protein, …), typo tolerance (`pg_trgm`), and search history are explicit follow-ups — see the plan at `~/.claude/plans/i-have-now-plenty-ticklish-raccoon.md`. |
