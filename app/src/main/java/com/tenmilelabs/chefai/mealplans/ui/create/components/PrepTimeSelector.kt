...
fun PrepTimeSelector(
    ...
    Column(modifier = modifier.fillMaxWidth()) {
        WizardSectionLabel(stringResource(R.string.wizard_prep_time_title))
        FlowRow(
            ...
            prepTimeOptions.forEach { minutes ->
                val label = if (minutes != null) {
                    stringResource(R.string.wizard_minutes_format, minutes)
                } else {
                    stringResource(R.string.wizard_no_limit)
                }
                FlatChip(
                    label = label,
